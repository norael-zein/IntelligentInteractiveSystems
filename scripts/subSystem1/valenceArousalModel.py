...
for target, y_train, y_test, y_train_val, y_test_val in [
    ...
    for model_name, model, param_grid in models_and_params:
        ...
        print(f"{model_name} - Best parameters: {current_model.best_params_}, Test R²: {test_r2}, Test MSE: {test_mse}")

        # Save the best model
        if not best_model or test_r2 > best_model["best_r2"]:
            best_model = {
    ...
    print(
        f"Best model for {target}: {best_model['model_name']} with parameters: {best_model['best_params']} and R²: {best_model['best_r2']}")

# Save the best models for valence and arousal
joblib.dump(best_models["valence"], "scripts/subSystem1/valence_model.pkl")
joblib.dump(best_models["arousal"], "scripts/subSystem1/arousal_model.pkl")
